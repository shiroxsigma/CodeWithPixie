...
- **Code モードの会話は自動保存**される（ワークスペースのサイドカー `.pixie_code_chat.json`）。
  チャット欄の **🗂 会話** から保存済み会話の一覧を開き、クリックで復元できる —
  チャット表示だけでなく**エンジンの文脈も引き継がれる**（`history_replace` でシード。
  ツール実行の詳細は本文だけに丸められる）。サーバ再起動後も続きから話せる。
...
| `favorites[]` | `[]` | ⭐ お気に入りフォルダ `{name, path}`（UI から登録） |
| `recent_workspaces[]` | `[]` | 🕘 最近使った作業フォルダ（切替時に自動追加・12件） |
...
- **フローチャートの直接編集**: プレビューの図に hover して **✏️ 編集**。編集モードでは
  図の上から直接操作でき、変更は**Mermaid ソースへの最小編集としてエディタに反映**される
  （`Ctrl+Z` で元に戻せる。mdflow のプリセット操作と同じ仕組み）。
  - ノード: クリックで選択、**ダブルクリックでラベルをその場編集**。`➕ ノード` で追加
    （追加直後にラベル編集が開く）、`🗑 削除` または `Delete` キーで削除
    （接続している矢印もまとめて削除）。
  - 矢印: クリック（またはダブルクリック）で選択してラベルをその場編集（空にすると削除）。
    選択中は `⇄ 反転` で向きを入れ替え、線種セレクタで `-->` / `-.->` / `==>` などに変更できる。
    `➕ 矢印` のあと始点→終点の順にノードをクリックして追加（自己ループも可）。
  - `Esc` で編集モードを終了、`Ctrl+Z` / `Ctrl+Y` は図の上からエディタの undo/redo に届く。
    操作したあとも選択は保たれるので、続けて編集できる。
  - ノードや矢印を消すと、それを指す `class` / `style` / `linkStyle` の参照も一緒に直す
    （`linkStyle` の番号がずれて図ごと描画エラーになるのを防ぐため）。
  - 対応するのは flowchart のよく使うサブセット（ノード定義・`-->` 系と `|ラベル|`・
    チェイン記法・classDef 等の passthrough）。**subgraph を含む図と flowchart 以外の図
    （sequenceDiagram 等）は編集不可**で、✏️ ボタンが理由付きで無効になる。
  - 長い図の生成も段階的: エージェントにまとめ作成を頼むと、骨子（見出し＋プレースホルダ）を
    先に書き、1ターンに数セクションずつ埋めていく（ローカルモデルの出力途切れで進捗が
    全損するのを防ぐため）。
...
- 右: ファイルツリー＋全文検索、下にエージェントチャット。
  - ツリーは**遅延読み込み**（VS Code 型）: 起動時はルート直下だけ取得し、**フォルダを
    展開したときその直下だけ**を取得する（`/api/files/list`）。数十万ファイルの
    ワークスペースでも初期表示は一瞬（node_modules 等は走査時に枝刈り。1ディレクトリ
    2万件の上限に達した場合は「⚠ 一覧は先頭2万件まで」を表示し、到達は全文検索で）。
  - 階層表示（📂 クリックで折りたたみ）。**ドラッグ&ドロップで移動**（一覧の余白へ落とすと最上位へ）、
    **右クリックで新規作成・名前変更/移動・削除**。
  - **コンテキストのチェックボックス**（Note/Code モード）: ファイルにチェックを入れると、
    送信時にその中身がエージェントへ同梱される（pptx/docx 等はテキスト抽出して同梱。
    1ファイル8000字・総量上限あり、超過分は注記して省略）。Code モードでは「開いている
    ファイル＋選択範囲」に加えて、最初から読ませておきたいファイルを指定できる。
  - 画像・PDF などエディタで開けないファイルも一覧に出て、クリックで **OS の既定アプリ**で開く。
  - **全文検索**はヒット行の**前後2行**も出す（ヒット1行だけでは直したい箇所か判断できないため）。
    クリックで該当行へジャンプ。`Aa` で大文字小文字の区別を切り替え（**検索と置換で共通**）。
  - **🔁 一括置換**: 検索でヒットしたファイルをまとめて置換する。`👁 プレビュー` は1バイトも
    書かずに「どのファイルの何行が、どう変わるか」を出す。`✓ すべて置換` の対象は
    **その検索でヒットしたファイルだけ**（件数上限で一覧から溢れたファイルを巻き込まない）。
    検索語も置換文字列も**リテラル**として扱う（`a.b` が `axb` に当たらない、`C:\data` を
    そのまま入れられる）。置換前の内容は下の 🕰 履歴 に残るので丸ごと戻せる。
...
- **承認バー**: 書き込み・コマンド実行など破壊操作の直前に、ツール名と**引数全文**を表示。
  ...
  - ファイル書き込み（`write_file` / `search_and_replace` / `replace_lines`）は、左ペインに
    **左＝現在 ／ 右＝書き込まれる内容 の差分が自動表示**される（サーバが実行前後の内容を
    計算。複数ファイルはタブで切替。巨大・バイナリは引数表示のみにフォールバック）。
  - **✓ 修正して承認**: 書き込みが単一ファイルの承認では、差分ビューの右ペインを直接編集して
    「✓ 修正して承認」できる。編集内容がそのまま書き込まれ、元のツール呼び出しは
    「その書き込みは完了済み」という案内付きで却下される（エージェントは二重書き込みせず
    後続ステップへ進む）。
- **⏹ 停止**: 実行中は送信ボタンが停止に変わり、進行中ターンを協調キャンセル（承認待ちも解除）。
- **⏪ 巻き戻し**（Code モード）: 各返信に hover で出る ⏪ で、**そのターンの前の状態へ
  ファイルを戻せる**。サーバが各ターン開始前にワークスペースのテキストファイルを
  スナップショット（メモリ保持・直近10ターンぶん、1ファイル1MB超や総量30MB超は
  部分対象）。スナップショットより後に**作られた**ファイルは消さずに残す。
  承認ゲートが「実行前」の保険、⏪ が「実行後」の保険。
- **🕰 履歴（ローカル履歴）**: 保存の**直前**の内容をワークスペース内 `.pixie_history/` に
  退避する。トップバーの 🕰 履歴 から版を選んで中身を確認し、`↩ この版に戻す` で復元できる。
  - 対象は **UI 経由の書き込み全部**（`Ctrl+S`・自動保存・一括置換・web2md）。エージェントの
    書き込みは ⏪ 巻き戻しが担当する（別系統）。⏪ がメモリ保持・直近10ターンなのに対し、
    こちらはディスク常駐で**アプリを再起動しても残る**。
  - 溜め方: 直近30世代はそのまま、それより古いものは**1日1世代**に間引く（総数120まで）。
    直近の世代が60秒以内なら新しい世代を作らない — Note モードの自動保存（2秒間隔）で
    履歴を食い潰し、肝心の「編集を始める前」が押し出されるのを防ぐため。
  - 戻すときは**戻す直前の内容も履歴に積む**ので、戻し間違えてもやり直せる。
  - レイアウトは元の相対パスをそのまま辿れる形（`.pixie_history/docs/spec.md/日時.md`）。
    最後の砦なので、アプリが起動しなくても Explorer から救出できることを優先している。
  - 切るときは `config.json` に `"history_enabled": false`。
- **外部変更の検知**: 開いた時点の更新時刻を持っておき、保存時に照合する。その間に他の
  エディタやエージェントがそのファイルを書き換えていたら、**書き込まずに**確認を出す
  （上書き／見送り）。上書きを選んだ場合、消える側（相手の変更）は必ず 🕰 履歴 に積まれる。
  見送ると自動保存も止まる（2秒ごとに同じ確認が出続けないように。`Ctrl+S` で再開）。
- **⭐ 行き先（お気に入り / 最近使ったフォルダ）**: トップバーの ⭐ から**1クリックで**
  作業フォルダを切り替える。📂 のダイアログ上部にも同じ一覧が出る（`☆` で登録/解除、
  `📂` は移動せず中を見るだけ）。どちらも `config.json`（`favorites` / `recent_workspaces`）に
  持つのでワークスペースを移しても残る。最近使ったは切替のたびに自動で溜まる（12件）。
- **◀ ▶ 🕘 ファイルの行き来**: `Alt+←` / `Alt+→` で前に開いていたファイルへ戻る・進む。
  `Ctrl+E`（🕘）で最近開いたファイルの一覧から選べる。仕様書とコードの往復にツリーを
  辿り直さなくてよい。※ `Ctrl+Tab` は Chrome がタブ切替に予約していてページ側から拾えないため
  `Ctrl+E` にしている。作業フォルダを切り替えると履歴は捨てる（別の木のパスなので）。
- **⚙️ 設定**: モデル/サーバの選択、**Copilot 連携の on/off**。
- **📂 フォルダ**: 作業フォルダを切替（会話ごとに別フォルダを扱える）。

> **自動保存は Note モードだけ**（打鍵の2秒後）。Code モードでは入れていない — エージェントが
> 同じファイルを直接書き換えるので、自動保存はエージェントの編集を黙って踏み潰しうるため。
> Code モードの保存は明示的に `Ctrl+S`。どちらのモードでも、保存の直前の内容は 🕰 履歴 に残る。
...
> **長い文書は段階的に執筆**: 複数ファイルのまとめ等、長い新規 Markdown を作る依頼では、
> エージェントはまず**骨子**（タイトル・見出し構造・各セクション1行のプレースホルダ）だけを書き、
> 以降のターンで1〜3セクションずつ search/replace で埋めていく（`NOTE_SYSTEM_SUFFIX` /
> `build_apply_prompt` で指示）。ローカルモデルは出力がトークン上限で途切れると
> そのターンの進捗が全損するため、一気に全文は書かせない。
...
### Code モードの進め方（📋 計画を先に / ⚡ 通常）
トップバーの切替ボタンで Code モード内のサブモードを選ぶ（選択はブラウザに記憶）:
- **⚡ 通常**（既定）: エージェントが自律的に実装（破壊操作は従来どおり承認制）。
- **📋 計画を先に**: 依頼ごとにまず**読み取り専用ツールで調査して実行計画を提示**する
  （そのターンは書き込みツールを提示しない＝ファイルは変わらない）。計画は Plan モードと
  同じ承認ビューに出る。「✓ この計画で実行」で**同じセッションの続き**として実装へ、
  「✕ 修正を依頼」かチャットへの追記で計画を練り直せる。Plan モードの Code モード内蔵版で、
...
| ファイル | 役割 |
|---|---|
| `app/engine_adapter.py` | **AWP との唯一の接点**。Code / Note / Plan を `AgentProfile` で構築し、型付きイベントの SSE 変換・承認ブリッジ・協調キャンセル・変更検知という **Web 固有部分**を担う |
| `pixie_core`（AWP 側）| AWP が公開する UI 非依存の埋め込み API。`AgentProfile` / `ContextPolicy` / `EngineEvent` / turn metrics を含む安定境界 |
...
| `app/search.py` | ripgrep 全文検索（前後行つき）＋ヒットしたファイルへの一括置換 |
...
1. ~~出力の**型付きイベント**再設計と、engine 内の直書き `print` 全廃。~~ **完了** —
   API 1.11の`EngineEvent`境界へ移行し、engine内通知も`output_fn`経由に統一した。
2. ~~AWPの`AgentProfile`公開後、Code / Note / Planのセッション構築を1本へ統合。~~ **完了** —
   API 1.11の共通プロファイル生成とネイティブイベント橋渡しを全3モードで使用する。
