...
- **フローチャートの直接編集**: プレビューの図に hover して **✏️ 編集**。編集モードでは
  ...
  - ノード: クリックで選択、**ダブルクリックでラベルをその場編集**。`➕ ノード` で追加
    （追加直後にラベル編集が開く）、`🗑 削除` で削除（接続している矢印もまとめて削除）。
  - 矢印: クリックで選択して `✏️ ラベル` でラベル編集（空にすると削除）。`➕ 矢印` のあと
    始点→終点の順にノードをクリックして追加。
  - 対応するのは flowchart のよく使うサブセット（ノード定義・`-->` 系と `|ラベル|`・
    チェイン記法・classDef 等の passthrough）。**subgraph を含む図は編集不可**（✏️ 押下時に案内）。
