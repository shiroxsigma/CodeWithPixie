...
- **mermaid 図の画像出力**: 図に hover すると右上に `🖼 保存` / `📋 コピー` / `白でコピー` が出る
  （プレビューとチャットの両方）。PNG（2倍解像度。通常コピーはエディタの背景色、白でコピーは白背景）にして、
  ワークスペースの `images/` へ保存するか、クリップボードへ載せる。`白でコピー` は白背景の PNG を
  クリップボードへ載せるので、白い資料やスライドへ貼り付けやすい。
