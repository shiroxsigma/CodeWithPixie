import { c as S, a as Si } from "./main-baGq8gSd.js";
...
const de = globalThis.jsyaml || null, rs = () => de !== null, Mi = "fill:#ff9999,stroke:#333,stroke-width:2px", $i = "fill:#2a2a2a,stroke:#555,color:#888", Tn = /^```[ \t]*(?:yaml[ \t]+)?mdflow-mapping[ \t]*\n([\s\S]*?)^```/gm, os = /^﻿?---[ \t]*\r?\n([\s\S]*?)\r?\n---[ \t]*\r?\n?/, _i = /^﻿?---[ \t]*\r?\n/, Ti = /^\s*%%\s*id\s*:\s*(\S+)/m;
function Ni(e) {
  const t = Ti.exec(e);
  return t ? t[1] : "";
}
function Ri(e) {
  ...
  if (de) {
    ...
    for (; (u = Tn.exec(s)) !== null; ) {
      ...
      const m = [];
      for (const [y, b] of Object.entries(g.presets || {})) {
        const x = b || {};
        m.push({
          name: String(y),
          when: String(x.when ?? ""),
      ...
      d.push({
        ...
        presets: m,
        activeStyle: String(h.active ?? Mi),
        inactiveStyle: String(h.inactive ?? $i)
      });
    }
  }
...
const Nn = /\b([A-Za-z_][\w-]*)\s*[\[({]/g, Rn = /([A-Za-z_][\w-]*)\s*(?:-{2,3}>|-{2,3}|={2,3}>|-\.->|-\.-)\s*(?:\|[^|]*\|\s*)?([A-Za-z_][\w-]*)/g, Bi = /^\s*(graph|flowchart)\b/i, In = /* @__PURE__ */ new Set([
  "graph",
  "flowchart",
  "subgraph",
  "end",
...
`)) {
    ...
    Rn.lastIndex = 0;
    let i;
    for (; (i = Rn.exec(s)) !== null; )
      for (const r of [i[1], i[2]]) In.has(r) || t.add(r);
    for (Nn.lastIndex = 0; (i = Nn.exec(s)) !== null; )
      In.has(i[1]) || t.add(i[1]);
  }
  return [...t];
}
function Hi(e, t, n, s = "mdflowActive", i = null) {
  const r = e.replace(/\n+$/, "");
  if (!t?.length) return { code: r, missing: [] };
  let a = [], c = [...new Set(t)];
  const d = as(e), u = d ? cs(e) : [];
  if (d) {
    const g = new Set(u);
...
function ji(e, t, n) {
  const s = An(t), i = n == null ? null : `${s}: ${An(n)}`, r = os.exec(e);
  if (!r) {
...
` };
  ...
  const a = _i.exec(e)[0].length, c = r[1], d = [];
  let u = a;
...
` };
  ...
  const m = Lt(d[g].raw), h = " ".repeat(m + 2);
  let y = -1, b = null;
  for (let v = g + 1; v < d.length; v++) {
    const L = d[v].raw.replace(/\r$/, "");
    if (L.trim() && Lt(L) <= m) break;
    if (L.trim() && (b == null && (b = /^\s*/.exec(L)[0]), Oi(L) === t)) {
      y = v;
      break;
    }
  }
...
async function ds(e, { scale: t = Wi, background: n = null } = {}) {
  const { width: s, height: i } = qi(e), r = e.cloneNode(!0);
  ...
  try {
    const d = await Ki(c), u = document.createElement("canvas");
...
const Gi = [
  ...
  { open: "==", closers: [["==>", "==>"], ["===", "==="], ["==x", "==x"], ["==o", "==o"]] }
], Zi = us.flatMap((e) => e.closers.map(([t]) => t)), Xi = /^\s*(?:flowchart(?:-elk)?|graph)(?:\s+[A-Za-z]{2})?\s*;?\s*(?:%%.*)?$/i, Ji = /^\s*(classDef|class|style|linkStyle|click|direction|accTitle|accDescr|title)\b/i, Qi = /^[A-Za-z0-9_\u0080-\uFFFF][A-Za-z0-9_.\-\u0080-\uFFFF]*/, er = /^(\s*class\s+)([^\s;]+)(\s+.*)$/i, tr = /^(\s*style\s+)([^\s;,]+)(\s+.*)$/i, nr = /^(\s*linkStyle\s+)(\d+(?:\s*,\s*\d+)*)(\s+.*)$/i;
function Jt(e) {
  const t = {
    supported: !0,
    reason: "",
...
function ir(e) {
  const t = Ge.get(e);
  if (t) return t;
  const n = Jt(e), s = { ok: n.supported, reason: n.reason };
  return Ge.size >= sr && Ge.clear(), Ge.set(e, s), s;
}
function Ye(e, t, n, s) {
  const i = { type: "other", start: n, end: s, indent: fs(t) };
  let r;
  (r = er.exec(t)) ? i.cls = {
...
function mr(e, t, n) {
  const s = e.edges[n];
  if (!s) return [];
  const i = e.statements[s.stmtIdx], r = gs(e, t, /* @__PURE__ */ new Set([n]));
  if (i.arrows.length === 1)
    ...
    }), r.push({ start: i.start, end: i.end, text: Qt(i, a, null) });
  }
  return gt(r);
}
function hr(e, t, n, s) {
...
function gr(e, t, n) {
  ...
  return [{
    ...
    text: Qt(i, r, null, s.indexInStmt)
  }];
}
function wr(e, t, { label: n = "新規ノード" } = {}) {
  const s = lr(e.nodes), { at: i, prefix: r } = ms(e);
...
function Pt(e, t, n, s = null) {
  ...
  }, d = c("ラベル編集", "選択中のノード/矢印のラベルを編集", vi);
  c("➕ ノード", "ノードを追加（追加後にラベルを編集できます）", yi);
  const u = c("➕ 矢印", "矢印を追加（始点→終点の順にノードをクリック）", Ei), p = c("⇄ 反転", "選択中の矢印の向きを入れ替える", bi), g = document.createElement("select");
  g.className = "mermaid-arrow-kind", g.title = "選択中の矢印の線種を変える";
  for (const [f, w] of xr) {
    const E = document.createElement("option");
    E.value = f, E.textContent = w, g.appendChild(E);
  }
  g.addEventListener("change", xi), a.appendChild(g);
  const m = document.createElement("select");
  m.className = "mermaid-node-shape", m.title = "選択中のノードの形状を変える";
  for (const [f, w, E] of kr) {
    const k = document.createElement("option");
    k.value = f + St + w, k.textContent = E, m.appendChild(k);
  }
  m.addEventListener("change", ki), a.appendChild(m);
  const h = c("削除", "選択中のノード/矢印を削除（Delete キーでも可。Ctrl+Z で戻せます）", Cn), y = document.createElement("span");
  ...
  const b = (f) => {
    ...
    const [w, E] = f || ["[", "]"], k = w + St + E;
    if (![...m.options].some((C) => C.value === k)) {
      const C = document.createElement("option");
      C.value = k, C.textContent = `${w}…${E}`, m.appendChild(C);
    }
    m.value = k;
  }, L = () => {
    if (!n.reveal) return;
    ...
    if (f.kind === "node") {
      const k = i.nodes.get(f.id), C = k?.def ? k.def.span : k?.firstRef?.span;
    ...
    const f = r.selected, w = f?.kind === "edge", E = f?.kind === "node";
    d.disabled = !f, h.disabled = !f, d.textContent = w ? "ラベル編集（矢印）" : "ラベル編集", p.classList.toggle("hidden", !w), g.classList.toggle("hidden", !w), m.classList.toggle("hidden", !E), u.classList.toggle("active", !!r.arrowPick), w && x(i.edges[f.idx]?.arrow.text || "-->"), E && v(i.nodes.get(f.id)?.def?.shape), r.arrowPick ? b(r.arrowPick.from ? `➕ 矢印: 始点 ${r.arrowPick.from} → 終点のノードをクリック（Escで中止）` : "➕ 矢印: 始点のノードをクリック（Escで中止）") : b(f ? f.kind === "node" ? `選択中: ノード ${f.id}（Delete で削除）` : "選択中: 矢印（Delete で削除）" : "クリック: 選択 ／ ダブルクリック: ラベル編集 ／ Esc: 終了"), L();
  };
  M();
  const N = () => {
    e.querySelectorAll(".selected").forEach((f) => f.classList.remove("selected")), r.selected = null;
  }, I = (f) => [...e.querySelectorAll("g[id*='flowchart-']")].find((w) => Je(w.id, i.nodes) === f) || null, P = (f) => [...e.querySelectorAll("path.flowchart-link")].find((w) => Qe(w, i) === f) || null, j = (f, w) => {
    N(), r.selected = { kind: "node", id: f }, (w || I(f))?.classList.add("selected"), M();
  }, D = (f, w) => {
    N(), r.selected = { kind: "edge", idx: f }, (w || P(f))?.classList.add("selected"), M();
  }, re = () => {
    const f = r.selected;
    if (!f) return null;
    ...
    if (r.busy || !f || !f.length) return;
    r.busy = !0, n.applyEdits(t.src, f, {
      editNodeId: w,
      selection: E === void 0 ? re() : E
    }) || Ce(!1);
  }, z = e.querySelector(":scope > .mermaid-canvas") || e, X = (f, w) => {
    const E = z.getBoundingClientRect();
    return { x: f - E.left + z.scrollLeft, y: w - E.top + z.scrollTop };
  }, he = (f) => {
    const w = f.getBoundingClientRect(), E = X(w.left, w.top);
  ...
  }, bt = ({ left: f, top: w, width: E, value: k, placeholder: C }, W) => {
    e.querySelectorAll(".mermaid-inline-input").forEach((te) => te.remove());
    const T = document.createElement("textarea");
    T.className = "mermaid-inline-input", T.rows = 1, T.value = k || "", C && (T.placeholder = C), T.style.left = `${Math.max(0, f)}px`, T.style.top = `${Math.max(0, w)}px`, T.style.width = `${Math.max(160, E)}px`, z.appendChild(T);
    const ge = () => {
    ...
    const E = Dn(i.nodes.get(w)?.def?.label ?? ""), k = he(f);
    bt({
      left: k.left,
      top: k.top,
      width: k.width + 24,
      ...
      C !== E && A(dr(i, t.src, w, Ct(C)));
    });
  }, Ln = (f, w) => {
    const E = Dn(i.edges[f]?.arrow?.label || ""), k = kn(w ? H(w) : null);
    bt(
      { ...k, value: E, placeholder: "矢印ラベル（Shift+Enter で改行・空で削除）" },
      (C) => {
        C !== E && A(fr(i, t.src, f, Ct(C)));
      }
    );
  };
  function vi() {
  ...
  function xi() {
    const f = r.selected;
    f?.kind === "edge" && A(hr(i, t.src, f.idx, g.value));
  }
  function ki() {
    const f = r.selected;
    if (f?.kind !== "node") return;
    const [w, E] = m.value.split(St);
    A(ur(i, t.src, f.id, w, E));
  }
  function Cn() {
    const f = r.selected;
    f && A(f.kind === "node" ? yr(i, t.src, f.id) : mr(i, t.src, f.idx), { selection: null });
  }
  const Li = (f) => {
    ...
    if (w) {
      const k = Je(w.id, i.nodes);
      if (!k) return;
      if (r.arrowPick) {
        if (!r.arrowPick.from)
          r.arrowPick = { from: k }, N(), w.classList.add("selected"), M();
        else {
          const C = r.arrowPick.from;
          r.arrowPick = null, N(), M(), bt(
            { ...wi(C, k), value: "", placeholder: "矢印ラベル（空でも可・Shift+Enter で改行）" },
            (W) => A(
    ...
    if (!E) return;
    const k = Qe(E, i);
    k != null && (f.preventDefault(), D(k, E), Ln(k, E));
  }, _n = (f) => {
    if (!e.isConnected) {
      Ce(!1);
      return;
    }
    if (!f.target?.closest?.(Lr)) {
      if (f.key === "Escape") {
        if (r.arrowPick) {
      ...
      if ((f.key === "Delete" || f.key === "Backspace") && r.selected) {
        f.preventDefault(), Cn();
        return;
      }
  ...
  e.addEventListener("click", Mn), e.addEventListener("dblclick", $n), document.addEventListener("keydown", _n);
  function Ce(f) {
    e.removeEventListener("click", Mn), e.removeEventListener("dblclick", $n), document.removeEventListener("keydown", _n), e.classList.remove("mermaid-editing"), a.remove(), e.querySelectorAll(".mermaid-inline-input").forEach((w) => w.remove()), N(), n.reveal?.(null), f && n.onExit?.();
  }
...
`), n = [];
  ...
  for (let r = 0; r < t.length; r++) {
    ...
    for (let u = r + 1; u < t.length; u++) {
      ...
      const g = n[u];
      i.push({ start: d, end: g, content: e.slice(d, g) }), r = u;
      break;
    }
  }
  return i;
...
`);
  ...
  for (let p = 0; p < n.length; p++) {
    ...
    h && !d && (d = h), i.push(a + h.length), r.push(c), a += n[p].length + 1, c += m.length + 1;
  }
  return { ...e, indent: d, toRaw: (p) => {
    let g = 0, m = r.length - 1;
    for (; g < m; ) {
      const h = g + m + 1 >> 1;
      r[h] <= p ? g = h : m = h - 1;
    }
...
function ws(e, t, n = 0) {
  const s = Sr(e), i = s[n] ? Fn(s[n], t) : null;
  if (i) return i;
  for (const r of s) {
    const a = Fn(r, t);
    if (a) return a;
...
${Mt(d.content)}</pre>`;
    ...
    for (const i of s.tokens_meta)
      i?.delimiters && Bn(s, i.delimiters);
    return !0;
...
let Ze = 0;
const Me = /* @__PURE__ */ new Map(), Tr = 50, rt = /* @__PURE__ */ new WeakMap(), Hn = /* @__PURE__ */ new WeakMap(), Xe = [0.5, 0.67, 0.8, 1, 1.25, 1.5, 2, 3], et = /* @__PURE__ */ new WeakMap();
function Nr(e, t) {
  return {
    get() {
...
function Br(e, t, n = null) {
  const s = Ui(t.src, t.index || 0), i = document.createElement("div");
  i.className = "mermaid-tools";
  const r = document.createElement("span");
  r.className = "mermaid-tools-status";
  const a = async (u, p, g) => {
    const m = u.textContent;
    u.disabled = !0, u.textContent = "⏳", r.className = "mermaid-tools-status", r.textContent = "";
    try {
      r.textContent = await g() || p;
    } catch (h) {
      r.className = "mermaid-tools-status error", r.textContent = h?.message || String(h);
    } finally {
      u.disabled = !1, u.textContent = m;
      const h = r.textContent;
      setTimeout(() => {
        r.textContent === h && (r.textContent = "", r.className = "mermaid-tools-status");
      }, 6e3);
    }
  }, c = () => ds(e.querySelector("svg"), { background: ys() });
  if (i.appendChild(r), n && e.__mermaidNatural && i.appendChild(Ir(e, n)), t.editable && Ft) {
    const { ok: u, reason: p } = ir(t.src), g = document.createElement("button");
    g.type = "button", g.title = u ? "この図を直接編集する（ノード/矢印の操作がMermaidソースへ反映される）" : `この図は直接編集できません（${p}）`, g.textContent = "編集", g.disabled = !u, u && g.addEventListener("click", () => Ft(e, t)), i.appendChild(g);
  }
  if (Dt) {
    const u = document.createElement("button");
    u.type = "button", u.title = "PNG にしてワークスペースへ保存する（同じ図は同じ名前へ書き直す）", u.textContent = "保存", u.addEventListener("click", () => a(u, "保存しました", async () => `✓ ${await Dt(await c(), s)}`)), i.appendChild(u);
  }
  const d = document.createElement("button");
  return d.type = "button", d.title = "PNG をクリップボードへコピーする", d.textContent = "コピー", d.addEventListener("click", () => a(d, "コピーしました", async () => (await zi(await c()), "✓ コピーしました"))), i.appendChild(d), i;
}
function ys() {
...
function On(e, t = {}, n = null) {
  ...
  return Number.isFinite(c) && Number.isFinite(d) && (s.__mermaidNatural = { w: c, h: d }), r && s.prepend(Br(s, t, n)), vs(s, n ? n.get() : 1), Pe && Pe(s, t), s;
}
async function Hr(e, t, n) {
  if (!Ee) return;
  const s = n.mdflow || null, i = Hn.get(e) || /* @__PURE__ */ new Map(), r = /* @__PURE__ */ new Map();
  Hn.set(e, r);
  let a = -1;
  for (const c of e.querySelectorAll("pre.mermaid-src")) {
    ...
    if (h && h.src === d && h.renderSrc === p) {
      h.meta.index = a, r.set(a, h), g(h.box), Pe && Pe(h.box, h.meta, !0);
      continue;
    }
    const y = (L) => (r.set(a, { src: d, renderSrc: p, box: L, meta: m }), L), b = Nr(e, m), x = Me.get(p);
    if (x) {
      g(y(On(x, m, b)));
      continue;
    }
    let v;
    try {
      ({ svg: v } = await Ee.render(`pixie-mermaid-${Ze++}`, p));
    } catch (L) {
      if (document.getElementById(`dpixie-mermaid-${Ze - 1}`)?.remove(), u && p !== d)
        try {
          p = d, { svg: v } = await Ee.render(`pixie-mermaid-${Ze++}`, p);
        } catch {
    ...
    if (Ar(p, v), rt.get(e) !== t) return;
    g(y(On(v, m, b)));
  }
}
function Or(e, t) {
  if (!rs()) return null;
  const n = Ni(e), s = Ii(t.doc.mappings, n);
  ...
  const c = t.doc.selected[n] || null, d = Fi(s, a ? {} : r, c);
  let u = e, p = [];
  d && ({ code: u, missing: p } = Hi(
    e,
    d.activeNodes,
    d.style,
    "mdflowActive",
...
function $(e, t, n = {}) {
  ...
  s.className = "msg " + e;
  const i = document.createElement("div");
  return i.className = "body", e === "assistant" ? je(i, t, n) : Es(i, t), s.appendChild(i), bs().appendChild(s), Y(e === "user"), s;
}
function xs(e, t) {
...
function Y(e = !1) {
  const t = bs();
  t && (e && (Bt = !0), Bt && (t.scrollTop = t.scrollHeight));
}
function Ht(e) {
...
function zr(e) {
  return ks(e).map(({ search: t, replace: n }) => ({ search: t, replace: n }));
}
function Gr(e) {
  if (e = Ht(e).visible, !e.trim()) return "";
...
const o = {
  ...
  vue: "html",
  svelte: "html"
}, wt = (e) => (e.split(".").pop() || "").toLowerCase(), en = (e) => Zr[wt(e)] || "plaintext", Ls = (e) => !!e && ["md", "markdown"].includes(wt(e)), Xr = () => "";
let Cs = /* @__PURE__ */ new Set([".pptx", ".docx", ".xlsx", ".pdf"]);
const tn = (e) => Cs.has("." + wt(e));
window.__monacoReady.then((e) => {
  o.monaco = e, o.editor = e.editor.create(l("editor"), {
    ...
    Ds(), Fs(), Vs();
  }), o.editor.onDidScrollChange(() => Gs()), o.editor.onDidChangeCursorSelection(ei), o.editor.addCommand(e.KeyMod.CtrlCmd | e.KeyCode.KeyS, () => jt()), o.editor.addCommand(
    e.KeyMod.CtrlCmd | e.KeyMod.Shift | e.KeyCode.KeyP,
    () => Vt()
  ), o.editor.addCommand(e.KeyMod.Alt | e.KeyCode.LeftArrow, () => Wt()), o.editor.addCommand(e.KeyMod.Alt | e.KeyCode.RightArrow, () => Ut()), o.editor.addCommand(e.KeyMod.CtrlCmd | e.KeyCode.KeyE, () => qt()), o.editor.onMouseDown((n) => {
  ...
  t.addEventListener("wheel", zs, { passive: !0, capture: !0 }), t.addEventListener("paste", (n) => {
    if (!R()) return;
    const s = Zn(n.clipboardData);
    s && (n.preventDefault(), n.stopPropagation(), Xn(s));
...
async function Jr() {
  Pr(jo), Dr(zo), Fr(Yo), await Ss(), nn(), await vt(), await V(), R() && await on(), Za();
}
async function Ss() {
  try {
    Ot(await ee("/api/mode"));
  } catch {
    Ot({ mode: "code", features: {} });
  }
}
const Re = ["code", "plan", "note"], De = { code: "Code", plan: "Plan", note: "Note" };
function Ot(e) {
  o.mode = Re.includes(e.mode) ? e.mode : "code", o.features = e.features || {}, Array.isArray(o.features.extract_exts) && (Cs = new Set(o.features.extract_exts)), o.copilotEnabled = !!o.features.copilot;
}
function nn() {
  const e = R(), t = l("mode-btn");
  for (const n of Re)
    document.body.classList.toggle("mode-" + n, o.mode === n), t.classList.toggle("mode-" + n, o.mode === n);
  t.textContent = De[o.mode], t.title = `現在: ${De[o.mode]} モード（クリックで次のモードへ切替）`, Ue() || fi(), o.editor?.updateOptions({ glyphMargin: e }), ei(), sn(), Ms(), qe();
}
function Ms() {
...
function Qr() {
  o.codeStyle = o.codeStyle === "plan" ? "normal" : "plan", localStorage.setItem("pixie.codeStyle", o.codeStyle), Ms(), $("system", o.codeStyle === "plan" ? "計画を先に: エージェントはまず実行計画を提示し、承認してから実装します。" : "通常: エージェントが自律的に実装します（破壊操作は従来どおり承認制）。");
}
function sn() {
...
（会話セッションはリセットされます）`)) return !1;
  ...
  try {
    ...
    return Ot(i), t.keepMessages || (l("messages").innerHTML = ""), l("approval").classList.add("hidden"), l("approval").innerHTML = "", o.assistantEl = null, o.sessionId = We(), Le(), $s(), nn(), await V(), R() ? (await on(), o.currentFile && (await gn(), await vn()), $("system", "Noteモードに切り替えました（読み取り専用エージェント・クリック反映）。")) : Ue() ? $("system", "Planモードに切り替えました（調べて実行計画を立てるだけ。承認するまでファイルは変更されません）。") : $("system", "Codeモードに切り替えました（自律エージェント・破壊操作は承認制）。"), !0;
  } catch (i) {
...
async function on() {
  ...
  for (const s of e.messages || []) {
    ...
    i._exchange = { userEl: t, userText: n }, xs(i, () => ri(i, 0)), t = null, n = "";
  }
  o.historyLoaded = !0;
}
async function an() {
...
async function vt() {
  try {
    ...
    l("agent-status").textContent = `  ・${e.tools} tools`, _s(e.workspace);
  } catch {
...
async function io(e) {
  ...
  for (const s of t.slice(0, -1)) {
    n = n ? n + "/" + s : s, o.fsMap.has(n) || await ot(cn(n));
...
function qe() {
  const e = l("file-list");
  e.innerHTML = "";
  const t = [...o.fsMap.values()].filter((n) => !so(cn(n.path))).sort((n, s) => n.path < s.path ? -1 : n.path > s.path ? 1 : 0);
  for (const n of t) {
    ...
    if (a.className = "fname", a.textContent = s[s.length - 1], n.type === "dir")
      ...
      i.addEventListener("click", () => n.text ? Q(n.path) : Ps(n.path));
    }
...
function co(e, t) {
  ...
  t.type === "dir" ? (s("中に新規ファイル", () => at("file", t.path + "/")), s("中に新規フォルダ", () => at("dir", t.path + "/"))) : t.text || s("↗ 既定のアプリで開く", () => Ps(t.path)), s("名前変更・移動", () => lo(t)), s("削除", () => uo(t)), n.style.left = e.pageX + "px", n.style.top = e.pageY + "px", document.body.appendChild(n);
}
...
async function at(e, t = "") {
  ...
  await ln("/api/fs/create", { path: i, kind: e }) && (e === "dir" && o.collapsedDirs.delete(i), await V(), e === "file" && await Q(i));
}
async function lo(e) {
  const t = prompt("新しいパス（フォルダに入れるには src/名前.py のように）", e.path);
  !t || !t.trim() || t.trim() === e.path || await Rs(e, t.trim().replace(/\\/g, "/"));
}
async function Rs(e, t) {
  ...
  o.checkedFiles = new Set([...o.checkedFiles].map(n)), Is(n), await V();
}
function Is(e) {
  const t = (n) => {
    const s = [];
...
async function As(e, t) {
  ...
  i !== e && e.split("/").slice(0, -1).join("/") !== t && await Rs(n, i);
}
async function uo(e) {
  confirm(`「${e.path}」を削除しますか？`) && await ln("/api/fs/delete", { path: e.path }) && (o.checkedFiles.delete(e.path), Is((t) => t === e.path ? null : t), o.currentFile === e.path && (o.currentFile = null, o.baseMtime = null, o.editor.setValue(""), yt(), U(), l("current-file").textContent = "（ファイル未選択）", ke(), xe(), R() && (await gn(), await vn())), await V());
}
async function Ps(e) {
  try {
...
async function Q(e, t, n = "push") {
  ...
  s && (n === "push" && o.currentFile && o.currentFile !== e && (o.navBack.push(o.currentFile), o.navFwd.length = 0), go(e), o.currentFile = e, o.baseMtime = s.mtime ?? null, o.conflictDeclined = !1, o.mdflowConditions.clear(), Go(), Rr(l("preview")), o.monaco.editor.setModelLanguage(o.editor.getModel(), en(e)), o.editor.setValue(s.content), o.saveError = null, yt(), U(), l("current-file").textContent = e, ke(), xe(), fn(), await io(e), qe(), R() && (await gn(), await vn()));
}
let Wn = null;
function yt() {
...
function U(e) {
  ...
  if (clearTimeout(Wn), t.classList.remove("save-error"), t.title = "", e === "saving") {
    t.textContent = "保存中…";
    return;
  }
  if (e === "saved") {
    t.textContent = "保存済", Wn = setTimeout(U, 1500);
    return;
  }
  if (o.saveError) {
...
async function fo() {
  ...
  if (o.currentFile === e && (o.baseMtime = c?.mtime ?? o.baseMtime), o.currentFile === e && (o.savedVersionId = n, Ds()), s && await V(), i)
    try {
      await wn(e, r);
    } catch (d) {
...
function jt() {
  return o.conflictDeclined = !1, dn();
}
const po = 2e3;
let ct = null;
function Fs() {
...
async function Ke() {
  clearTimeout(ct), R() && (o.conflictDeclined || o.currentFile && o.dirty && await dn());
}
...
const Wt = () => Bs(o.navFwd, o.navBack), Ut = () => Bs(o.navBack, o.navFwd);
function qt() {
  const e = o.navRecent.filter((t) => t !== o.currentFile).map((t) => ({ label: t, title: t, onClick: () => Q(t) }));
  e.length && Ns(l("recent-btn"), [{ label: "最近開いたファイル" }, ...e]);
}
let Be = null;
async function vo() {
...
async function Eo(e, t, n) {
  ...
  !s || Be !== t.id || (l("hist-preview").textContent = s.content, l("hist-preview-head").textContent = `${Hs(t.saved_at)} の内容`, l("hist-restore").disabled = !1);
}
async function bo() {
...
const js = (e) => B.favorites.some((t) => xo(t.path, e)), xo = (e, t) => String(e || "").replace(/[\\/]+$/, "").toLowerCase() === String(t || "").replace(/[\\/]+$/, "").toLowerCase();
async function ko() {
  await Os();
  const e = [], t = (n, s) => n.map((i) => ({
    label: `${s} ${i.name}${i.exists ? "" : "（見つかりません）"}`,
    title: i.path,
    onClick: i.exists ? () => bn(i.path) : void 0
  }));
  B.favorites.length && e.push({ label: "お気に入り" }, ...t(B.favorites, "⭐")), B.recent.length && e.push({ label: "最近使ったフォルダ" }, ...t(B.recent, "🕘")), e.push({ label: "フォルダを選ぶ…", onClick: Xt }), !B.favorites.length && !B.recent.length && e.unshift({ label: "行き先はまだありません（フォルダを移動すると溜まります）" }), Ns(l("places-btn"), e);
}
function Ws() {
  ...
  const t = (n, s, i, r) => {
    ...
    for (const c of s) {
      ...
      const u = document.createElement("button");
      u.className = "place-go", u.textContent = `${i} ${c.name}`, u.title = c.exists ? `${c.path}（クリックでここへ移動）` : `${c.path}（見つかりません）`, u.disabled = !c.exists, u.addEventListener("click", () => bn(c.path));
      const p = document.createElement("button");
      p.className = "place-mini", p.textContent = "開く", p.title = "移動せずに中を見る", p.disabled = !c.exists, p.addEventListener("click", () => be(c.path));
      const g = document.createElement("button");
      g.className = "place-mini", g.textContent = r ? "★" : "☆", g.title = r ? "お気に入りから外す" : "お気に入りに入れる", g.addEventListener("click", () => Us(c.path, c.name)), d.append(u, p, g), e.appendChild(d);
    }
  };
...
function un() {
  const e = l("root-input").value.trim(), t = l("root-fav-btn"), n = !!e && js(e);
...
function ke() {
  ...
  t.disabled = !(e && Z()), t.title = e && Z() ? "プレビューの内容をリッチテキスト（HTML）とMarkdownでコピー。Confluence 等に貼り付け可" : "Markdown プレビュー表示中に使えます", !e && Z() && Qs();
}
function Ks() {
  const e = o.editor.getValue();
  let t = e, n = 0;
  const s = {};
  if (R() && o.features.mdflow && rs())
    try {
      const i = Ri(e);
...
function fn() {
  if (!Z()) return;
  $r(Ve());
  const { text: e, opts: t, lineOffset: n } = Ks();
  t.editable = !0, t.sourceMap = !0, ut = n;
  const s = performance.now(), i = je(l("preview"), e, t);
  Gs(), Promise.resolve(i).then(() => {
    qs = performance.now() - s;
  });
}
function So() {
  l("preview").addEventListener("click", (e) => {
    ...
    if (!n) return;
    const s = t.dataset.preset || null, i = ji(o.editor.getValue(), n, s);
    if (!i) return;
    ...
    if (!t) return;
    const n = t.closest(".mdflow-presets")?.dataset.diagram;
    n && (o.mdflowConditions.set(n, t.value), Vs());
  });
}
function Vs() {
  if (!Z()) return;
...
let lt = "", qn = null;
function pn(e) {
  lt = e, clearTimeout(qn), qn = setTimeout(() => {
    lt = "";
  }, Mo);
}
const $o = 200;
let dt = !1, Kn = null;
function zs() {
  dt = !0, clearTimeout(Kn), Kn = setTimeout(() => {
    dt = !1;
  }, $o);
}
function Gs() {
  ...
  pn("editor"), s.scrollTop = n * (s.scrollHeight - s.clientHeight);
}
function _o() {
  if (dt || !Z() || lt === "editor" || !o.editor) return;
  const e = l("preview"), t = e.scrollHeight - e.clientHeight, n = t > 0 ? e.scrollTop / t : 0, s = o.editor;
  pn("preview"), s.setScrollTop(n * Math.max(0, s.getScrollHeight() - s.getLayoutInfo().height));
}
const To = 80, No = 300;
let ut = 0, Vn = null;
function Ro() {
  ...
  n.textRange && s.push({ range: n.textRange, options: { className: "preview-src-hl" } }), o.previewHl ? o.previewHl.set(s) : o.previewHl = e.createDecorationsCollection(s), Zs(t, n.textRange), pn("preview"), e.revealRangeInCenterIfOutsideViewport(n.textRange || n.lineRange, 1);
}
function mn() {
...
function Io(e) {
  ...
  const i = s.querySelectorAll("[data-src-line]"), r = zn(n.startContainer) || i[0], a = zn(n.endContainer) || i[i.length - 1] || r;
  if (!r || !a) return null;
  const c = Kt(e, Number(r.dataset.srcLine) + ut), d = Math.max(c, Kt(e, Number(a.dataset.srcEnd) + ut));
  if (!c) return null;
  const u = new o.monaco.Range(c, 1, d, e.getLineMaxColumn(d)), p = Do(n), g = p ? p.textContent : t.toString(), m = Ao(e, n, g);
  if (m) return m;
  const h = Ys(e.getValueInRange(u), g);
  let y = null;
  if (h) {
...
function Ao(e, t, n) {
  ...
  if (!c) return null;
  const p = Number(s.dataset.srcLine) + ut, g = Kt(e, p + 1);
  if (!g) return null;
  const m = e.getOffsetAt({ lineNumber: g, column: 1 }), h = r.src.slice(c.start, c.end), y = Ys(h, n), b = m + c.start + (y?.start || 0), x = m + c.start + (y?.end ?? h.length), v = o.monaco.Range.fromPositions(
    e.getPositionAt(b),
    e.getPositionAt(x)
...
function Fo() {
  ...
  return e || (e = document.createElement("button"), e.id = "mark-btn", e.className = "hidden", e.title = "選択したところを Markdown の ==マーカー== で塗る (Ctrl+Shift+H)", e.addEventListener("mousedown", (t) => t.preventDefault()), e.addEventListener("click", Xs), document.body.appendChild(e), e);
}
function Zs(e, t) {
...
function Js(e) {
  l("preview").classList.toggle("hidden", !e), l("preview-divider").classList.toggle("hidden", !e), l("preview-btn").classList.toggle("active", e), e ? tc() : (l("editor").style.flex = "", mn()), o.editor?.layout(), ke();
}
function Qs() {
  Js(!1);
}
async function jo(e, t) {
  const n = o.currentFile || "", s = ls(mt(n).replace(/\.[^.]+$/, "")), i = await _("/api/image", {
    note: n,
    name: `${s}-${t}`,
    ext: "png",
    data_b64: await Vi(e),
...
async function Uo(e) {
  ...
  for (const n of t.querySelectorAll(".mermaid-box")) {
    ...
    if (s)
      try {
        const i = await ds(s, { background: ys() }), r = document.createElement("img");
        r.src = await Gn(i), r.style.maxWidth = "100%", n.replaceWith(r);
      } catch {
      }
  }
...
async function qo() {
  ...
  try {
    const { text: n } = Ks(), s = await Uo(l("preview"));
...
function zo(e, t) {
  ...
  o.diagramEditing = n, n.dispose = Pt(e, t, zt(), null);
}
function Go() {
...
function Yo(e, t, n = !1) {
  ...
  if (s) {
    if (n) {
      if (s.box !== e) return;
      s.index = t.index, e.querySelector(":scope > .mermaid-editbar") || (s.dispose?.(), s.dispose = Pt(e, t, zt(), s.restore));
      return;
    }
    s.index === t.index && (s.src !== t.src && (s.src = t.src, s.restore = null), s.dispose?.(), s.box = e, s.dispose = Pt(e, t, zt(), s.restore), s.restore && (s.restore = { ...s.restore, editNodeId: null }));
  }
}
function zt() {
  return {
    applyEdits(e, t, n = null) {
      const s = o.editor, i = s.getModel(), r = o.diagramEditing, a = ws(i.getValue(), e, r?.index ?? 0);
      if (!a)
...
function Yn(e) {
  ...
  const i = o.diagramEditing, r = i ? ws(n.getValue(), i.src, i.index ?? 0) : null;
  if (!r) {
    s();
    return;
  }
  const a = (p) => p ? o.monaco.Range.fromPositions(
    n.getPositionAt(r.start + r.toRaw(p.start)),
    n.getPositionAt(r.start + r.toRaw(p.end))
...
let Gt = null, ae = [];
function Zo() {
  clearTimeout(Gt), Gt = setTimeout(() => hn(l("file-search").value), 250);
}
function Xo() {
  clearTimeout(Gt), l("file-search").value = "", ae = [], l("search-results").classList.add("hidden"), l("search-results").innerHTML = "", l("search-opts").classList.add("hidden"), l("replace-bar").classList.add("hidden"), l("replace-preview").innerHTML = "", l("replace-status").textContent = "", l("file-list").classList.remove("hidden");
}
async function hn(e) {
...
置換前の内容は 🕰 履歴 に残るので元に戻せます。実行しますか？`
  ...
  o.currentFile && r.includes(o.currentFile) && await Q(o.currentFile, !0, "none"), await hn(l("file-search").value), ye(i);
}
function ea(e) {
...
async function wn(e = o.currentFile, t) {
...
function na() {
  ...
  t && (o.notes = o.notes.filter((n) => n.line !== e), o.notes.push({ line: e, text: t }), pt(), wn().catch((n) => alert("⚠️ 付箋の保存に失敗しました: " + n.message)));
}
function sa(e) {
  Et();
  const t = o.notes.find((s) => s.line === e), n = prompt("付箋メモ（空で削除）", t ? t.text : "");
  n !== null && (o.notes = o.notes.filter((s) => s.line !== e), n.trim() && o.notes.push({ line: e, text: n }), pt(), wn().catch((s) => alert("⚠️ 付箋の保存に失敗しました: " + s.message)));
}
const ia = /* @__PURE__ */ new Set(
  ["md", "markdown", "txt", "py", "json", "yaml", "yml", "toml", "csv", "html", "css", "js", "ts"]
), ce = (e) => (e.external ? "E:" : "I:") + e.path, mt = (e) => e.split(/[\\/]/).pop(), Yt = (e) => ia.has(wt(e.path)), ti = (e) => tn(e.path);
async function vn() {
  if (!o.currentFile) {
    o.refs = [], He();
    return;
...
async function Zt(e) {
  ...
  o.refs.some((t) => ce(t) === ce(e)) || (o.refs.push(e), await ni(), He());
}
async function ra(e) {
  const [t] = o.refs.splice(e, 1);
  t && o.checkedRefs.delete(ce(t)), await ni(), He();
}
async function oa(e) {
  try {
    await _(
...
function He() {
  ...
  t.classList.add("hidden"), o.refs.forEach((n, s) => {
    ...
    r.type = "checkbox", r.title = Yt(n) ? "AIコンテキストに含める" : ti(n) ? "AIコンテキストに含める（サーバでテキスト抽出して同梱。/copilot では原本を Copilot に添付）" : "AIコンテキストに含める（この形式は Copilot 添付経路のみ有効）", r.checked = o.checkedRefs.has(ce(n)), r.addEventListener("click", (u) => u.stopPropagation()), r.addEventListener("change", () => {
...
function ca() {
  ...
  e.addEventListener("dragover", (t) => {
    ...
    if (J) {
      ...
      s && s.type === "file" ? await Zt({ path: J, external: !1, name: mt(J) }) : alert("フォルダは参照に追加できません。ファイルをドラッグしてください。");
      return;
    }
    const n = aa(t.dataTransfer.getData("text/uri-list") || t.dataTransfer.getData("text/plain"));
    if (n) {
      await Zt({ path: n, external: !0, name: mt(n) });
      return;
    }
...
async function Ie(e) {
  ...
  for (const i of t.files || []) {
    ...
    r.className = "pick-file", r.textContent = i.name, r.addEventListener("click", async () => {
      await Zt({ path: i.path.replace(/\\/g, "/"), external: !0, name: i.name }), st();
    }), s.appendChild(r);
  }
}
function la() {
...
function fa(e) {
  ...
  const g = (h, y = !1) => {
    ...
    p.querySelector("summary").textContent = y ? `💭 思考ログ（${h.length}文字・${b}s）` : "💭 思考中…", p.querySelector(".think-body").textContent = h, y && (p.open = !1);
  };
  a(), c();
  const m = setInterval(() => {
    c(), Y();
  ...
  return {
    onToken(h) {
      t += h, i.remove();
      const { think: y, visible: b } = Ht(t);
      g(y), Es(e.querySelector(".body"), b), Y();
    },
    /** エンジンのインジケータ（⏳ Prefill / 🧠 Thinking...）を待機表示のフェーズに反映する。 */
    setPhase: d,
    finish() {
      clearInterval(m), i.remove();
      const { think: h, visible: y } = Ht(t);
      return g(h, !0), y.trim() && je(e.querySelector(".body"), y, { assetBase: Ve() }), y;
    }
  };
}
const pa = [
...
async function ha(e, t) {
  ...
  for (const a of [...o.checkedFiles]) {
    ...
    c && c.content != null && i.push({ path: a, content: c.content }), tn(a) && s.push(a);
  }
  const r = [];
  if (o.currentFile)
...
function wa() {
  const e = [
    ...Object.entries(ii),
    ...Object.entries(yn).map(([t, n]) => [t, n.desc])
  ].map(([t, n]) => `| \`${t}\` | ${n} |`);
  $("assistant", [
    "### 使えるコマンド",
    "",
...
async function Oe() {
  ...
  try {
    ...
    const p = $("user", t);
    o.changedPaths.size && (o.changedPaths.clear(), qe()), o.assistantEl = $("assistant", ""), o.turnId = 0, o.compacted = null, o.assistantEl._exchange = { userEl: p, userText: t }, o.assistantUi = fa(o.assistantEl);
    try {
      ...
      S.phase(s, "running"), await Si(b, s, S.current, async (x) => {
    ...
    const g = s.controller.signal.aborted || s.outcome === "cancelled" || !!i, m = o.assistantEl, h = o.turnId, y = Jn();
    r ? La(m, t, y, d, g) : (a || c) && !g && ka(y), Ne() && !g && Ta(t, y), o.compacted && m?.isConnected ? ba(m, o.compacted) : m?.isConnected && (xs(m, () => ri(m, h)), Ne() && h && Vr(m, () => _a(h)));
  } catch (r) {
    s.controller.signal.aborted || (i = r.message, $("error", r.message));
  } finally {
    await s.interruption, S.current(s) && (o.assistantUi && Jn(), l("approval").classList.add("hidden"), l("approval").innerHTML = "", me.length && ie(), S.finish(s, i));
  }
}
...
function ka(e) {
  let t = xa(e);
  !t && /^\s*1[.)]\s/m.test(e || "") && (t = (e || "").trim()), t && ui(t);
}
function La(e, t, n, s, i) {
  if (o.history.push({ role: "user", content: t }), (n.trim() || !i) && o.history.push({ role: "assistant", content: n }), an(), !e || !e.isConnected) return;
  const r = zr(n);
...
` + m), t.appendChild(p);
  ...
  c.className = "btn-approve", c.textContent = "✓ 承認して実行", c.disabled = !!e.changeset && !e.changeset.ok, c.disabled && (c.title = "競合または検証エラーがあるため承認できません"), c.onclick = () => Qn(e.id, !0, null);
  const d = document.createElement("button");
  d.className = "btn-reject", d.textContent = "✗ 却下", d.onclick = () => Qn(e.id, !1, a.value.trim() || null), r.append(a, c, d), t.appendChild(r), Y();
}
...
（このターンより後に作られたファイルは消さずに残ります。）`))
    try {
      ...
      $("system", t.restored.length ? `${t.restored.length}件を巻き戻しました: ${t.restored.join(", ")}` : "戻す変更はありませんでした（既にターン前の内容と同じです）。"), t.restored.length && await ai(t.restored);
    } catch (t) {
...
`)[0]}`);
  ...
  i.length && (r += ` ⚠ mdflow: ${i.length}件の警告`), En(n, s.content, (a) => {
...
function Da(e, t, n) {
  ...
  i.className = "insert-btn", i.textContent = "▶ エディタへ反映", i.title = "このメッセージの提案を差分プレビューで確認してから反映する", i.addEventListener("click", () => di(Gr(t), n)), s.appendChild(i), e.appendChild(s), Y();
}
function Fa(e, t, n) {
  ...
  const c = document.createElement("button");
  return c.className = "apply-btn", c.textContent = "▶ 差分で反映", c.addEventListener("click", () => di(i, n)), a.appendChild(c), e.append(r, a), Y(), !0;
}
function di(e, t) {
...
function En(e, t, n, s, i = {}) {
  ...
  const a = i.lang || (o.currentFile ? en(o.currentFile) : "markdown"), c = r.editor.createModel(e, a), d = r.editor.createModel(t, a);
...
function Ha(e, t = null) {
  ...
  n.innerHTML = "", e.length > 1 && e.forEach((s, i) => {
    const r = document.createElement("button");
    r.type = "button", r.textContent = (s.path || "").split("/").pop() || s.path, r.title = s.path, r.addEventListener("click", () => es(i)), n.appendChild(r);
  }), es(0);
}
function es(e) {
  const t = me[e];
  t && ([...l("diff-tabs").children].forEach((n, s) => n.classList.toggle("active", s === e)), En(
    t.before,
    t.after,
    null,
    `承認確認: ${t.path}（左＝現在 ／ 右＝書き込まれる内容${pe ? "・右を編集して修正して承認できます" : ""}）`,
    { approval: !0, editable: !!pe, lang: en(t.path || "") }
  ));
}
async function Oa(e, t, n) {
  return oi("/api/approve-edit", { id: e, path: t, content: n }, "✓ 修正して承認しました（編集内容を適用）。");
}
function ui(e) {
...
` + e, await Oe();
  ...
  if (!await rn("code", { keepMessages: !0 })) {
    ui(e);
    return;
  }
...
async function Xt() {
  l("root-modal").classList.remove("hidden"), await Os(), Ws(), await be(l("root-path").textContent || "");
}
function it() {
  l("root-modal").classList.add("hidden");
}
const Wa = () => bn(l("root-input").value.trim());
async function bn(e) {
  if (!e) return;
  if (o.streaming) {
...
async function hi() {
  ...
  l("settings-copilot").checked = !!e.enabled, o.copilotEnabled = !!e.enabled, sn();
  const t = [];
...
function Za() {
  l("send-btn").addEventListener("click", () => o.streaming ? $a() : Oe()), l("new-session-btn").addEventListener("click", ci), l("sessions-btn").addEventListener("click", li), l("sessions-close").addEventListener("click", () => l("sessions-modal").classList.add("hidden")), l("sessions-modal").addEventListener("click", (e) => {
    e.target === l("sessions-modal") && l("sessions-modal").classList.add("hidden");
  }), Le(), ke(), l("save-btn").addEventListener("click", () => jt()), l("preview-btn").addEventListener("click", Vt), l("richcopy-btn").addEventListener("click", qo), l("preview").addEventListener("wheel", zs, { passive: !0 }), l("preview").addEventListener("scroll", () => {
    _o(), se && Zs(o.editor.getModel(), se.range);
  }, { passive: !0 }), document.addEventListener("selectionchange", () => {
    clearTimeout(Vn), Vn = setTimeout(Ro, To);
  }), So(), Vo(), l("refresh-btn").addEventListener("click", () => V()), l("file-search").addEventListener("input", Zo), l("search-case").addEventListener("change", () => hn(l("file-search").value)), l("replace-toggle").addEventListener("click", Qo), l("replace-preview-btn").addEventListener("click", () => Tt(!0)), l("replace-run-btn").addEventListener("click", () => Tt(!1)), l("replace-input").addEventListener("keydown", (e) => {
    e.key === "Enter" && (e.preventDefault(), Tt(!0));
  }), l("nav-back").addEventListener("click", Wt), l("nav-fwd").addEventListener("click", Ut), l("recent-btn").addEventListener("click", (e) => {
  ...
  }), xe(), l("mode-btn").addEventListener("click", eo), l("code-style-btn").addEventListener("click", Qr), l("plan-approve").addEventListener("click", ja), l("plan-reject").addEventListener("click", ts), l("note-btn").addEventListener("click", na), l("chat-clear-btn").addEventListener("click", to), ca(), l("ref-add-btn").addEventListener("click", la), l("pick-cancel").addEventListener("click", st), l("pick-input").addEventListener("keydown", (e) => {
  ...
  }), l("new-file-btn").addEventListener("click", () => at("file")), l("new-folder-btn").addEventListener("click", () => at("dir")), l("web2md-btn").addEventListener("click", Ua), l("cp-bar-open-btn").addEventListener("click", qa), l("cp-bar-import-btn").addEventListener("click", Ka), document.addEventListener("click", Fe), oo(), l("root-project-btn").addEventListener("click", Xt), l("folder-btn").addEventListener("click", Xt), l("root-cancel").addEventListener("click", it), l("root-ok").addEventListener("click", Wa), l("places-btn").addEventListener("click", (e) => {
    e.stopPropagation(), ko();
  }), l("root-fav-btn").addEventListener("click", () => {
    const e = l("root-input").value.trim();
    e && Us(e);
  }), l("root-input").addEventListener("input", un), l("root-input").addEventListener("keydown", (e) => {
    e.key === "Enter" && (e.preventDefault(), be(l("root-input").value.trim()));
  }), l("root-modal").addEventListener("click", (e) => {
    e.target === l("root-modal") && it();
  }), l("settings-btn").addEventListener("click", Va), l("settings-close").addEventListener("click", It), l("settings-think-budget-save").addEventListener("click", ns), l("settings-think-budget").addEventListener("keydown", (e) => {
    e.key === "Enter" && (e.preventDefault(), ns());
  }), l("settings-context-length-save").addEventListener("click", ss), l("settings-context-length").addEventListener("keydown", (e) => {
    e.key === "Enter" && (e.preventDefault(), ss());
    ...
    if (e.altKey && !e.ctrlKey && !e.metaKey && (e.key === "ArrowLeft" || e.key === "ArrowRight")) {
      e.preventDefault(), e.key === "ArrowLeft" ? Wt() : Ut();
      return;
    }
...
function gi({ divider: e, pane: t, container: n, min: s, key: i, after: r }) {
  const a = (p) => {
    const g = n.clientWidth - s - e.offsetWidth;
    t.style.flex = `0 0 ${Math.max(s, Math.min(p, Math.max(s, g)))}px`, o.editor?.layout(), r?.();
  }, c = () => {
...
const At = "pixie.filemgrHeight", is = 80;
function ic() {
  ...
  e.addEventListener("mousedown", (r) => {
    ...
    n(Math.max(is, Math.min(r.clientY - a, c)));
  }), e.addEventListener("dblclick", () => {
